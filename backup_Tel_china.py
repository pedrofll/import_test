...
MARCAS_VALIDAS = [
    "Xiaomi", "Redmi", "Samsung", "Apple", "Meizu", "Redmagic", "Tcl",
    "Oppo", "Oneplus", "Vivo", "Nubia", "Motorola", "Honor",
    "Realme", "Poco", "Google", "Nothing", "Huawei"
]

MARCAS_VALIDAS_LOWER = {m.lower(): m for m in MARCAS_VALIDAS}

def normalizar_capitalizacion_nombre(nombre):
    partes = []
    for tok in re.sub(r'\s+', ' ', str(nombre or '')).strip().split(' '):
        limpio = tok.strip()
        if not limpio:
            continue
        base = re.sub(r'[^A-Za-z0-9]', '', limpio)
        if any(c.isdigit() for c in base) and any(c.isalpha() for c in base):
            partes.append(limpio.upper())
        elif limpio.lower() in MARCAS_VALIDAS_LOWER:
            partes.append(MARCAS_VALIDAS_LOWER[limpio.lower()])
        else:
            partes.append(limpio[:1].upper() + limpio[1:].lower() if limpio else limpio)
    return ' '.join(partes).strip()

def limpiar_nombre_telegram(linea):
    """
    Extrae el nombre real desde una línea Telegram tipo:
    ♦️[EXCLUSIVA] ▫️ Oppo Find X9 Ultra -> Oppo Find X9 Ultra
    ♦️[ OFERTA FLASH] 🛑 Realme 14 Pro -> Realme 14 Pro
    ♦️[HISTORICO] 🛑 Realme 14T -> Realme 14T

    Regla fuerte: el nombre empieza en la primera marca válida detectada.
    Todo lo anterior se considera texto promocional/iconos.
    """
    if not linea:
        return ""

    s = str(linea).replace('**', '').replace('`', '').strip()
    s = re.sub(r'\s+', ' ', s).strip()

    # Quitar etiquetas entre corchetes, aunque estén pegadas a iconos.
    s_sin_tags = re.sub(r'\[[^\]]{0,80}\]', ' ', s, flags=re.I)
    s_sin_tags = re.sub(r'\s+', ' ', s_sin_tags).strip()

    # Buscar marca válida en la línea limpia y, si no, en la línea original.
    patron_marcas = r'\b(' + '|'.join(re.escape(m) for m in MARCAS_VALIDAS) + r')\b'
    m = re.search(patron_marcas, s_sin_tags, flags=re.I)
    if not m:
        m = re.search(patron_marcas, s, flags=re.I)
        fuente = s
    else:
        fuente = s_sin_tags
    ...
    nombre = fuente[m.start():].strip()

    # Cortes defensivos: evita que se arrastren campos posteriores si vienen en la misma línea.
    nombre = re.split(r'\s+(?:Precio|Cup[oó]n|Cod\.?\s*Promo|Link|RAM|ROM|Visita|S[ií]guenos)\b', nombre, maxsplit=1, flags=re.I)[0]

    # Quitar iconos/decoración final como ℹ️ que puede convertirse en i por Unicode.
    nombre = re.sub(r'[\s\u200b-\u200f\u2060\ufeff]+', ' ', nombre).strip()
    nombre = re.sub(r'[🔥⚡♦🆕📱📦🧡🧊🔘▪▫▶►⭐✅💥✨🚀ℹ🛑]+$', '', nombre).strip()
    nombre = re.sub(r'\s+i$', '', nombre, flags=re.I).strip()
    nombre = re.sub(r'^[^A-Za-z0-9ÁÉÍÓÚáéíóúÑñ]+', '', nombre).strip()
    nombre = re.sub(r'[^A-Za-z0-9ÁÉÍÓÚáéíóúÑñ\.\+\- ]+$', '', nombre).strip()
    nombre = re.sub(r'\s+', ' ', nombre).strip()

    return normalizar_capitalizacion_nombre(nombre)

def categoria_valida(nombre):
    marca = (str(nombre or '').split(' ')[0] if nombre else '').strip().lower()
    return marca in MARCAS_VALIDAS_LOWER


def enviar_email(asunto, cuerpo):
    print(f"\n📧 ENVIANDO EMAIL: {asunto}")
...
def extraer_datos(texto):
    ...
    for linea in lineas:
        ...
        if "http" in low or "www." in low:
            continue
        if re.search(r'\b(precio|cup[oó]n|cod\.?\s*promo|link|ram|rom|global version|eu version|visita|s[ií]guenos|facebook|instagram)\b', low, re.I):
            continue
        cand = limpiar_nombre_telegram(linea)
        if cand:
            nombre = cand
            linea_origen = linea
            break
    ...
    if not nombre:
        return ("SKIP_INVALID_BRAND", lineas[0] if lineas else "")

    nombre_upper = nombre.upper()
    if re.search(r'\b(PAD|IPAD|TAB)\b', nombre_upper):
        return "SKIP_TABLET"

    if not categoria_valida(nombre):
        return ("SKIP_INVALID_BRAND", linea_origen or nombre)
    ...
    m_c = re.search(r'(?:Cod\.\s*Promo|Cupón|Cupon|Código|Codigo):?\s*([A-Z0-9]+)', t_clean, re.I)
...
async def main():
    ...
    for msg in mensajes:
        ...
        if not res_data:
            continue
        if res_data == "SKIP_TABLET":
            print("⛔ IGNORADO TABLET/IPAD/TAB")
            continue
        if res_data == "SKIP_SPECS":
            print("⛔ IGNORADO SIN MEMORIA/CAPACIDAD")
            continue
        if isinstance(res_data, tuple) and len(res_data) == 2 and res_data[0] == "SKIP_INVALID_BRAND":
            print(f"⛔ IGNORADO CATEGORÍA NO VÁLIDA: {res_data[1]}")
            continue
        
        nombre, memoria, capacidad, version, codigo_de_descuento, precio_actual = res_data
        if not categoria_valida(nombre):
            print(f"⛔ IGNORADO CATEGORÍA NO VÁLIDA: {nombre}")
            continue
        ...
        if fuente == "Amazon": url_sin_acortar_con_mi_afiliado += "?tag=ofertasdemo04-21"
        ...
        marca = MARCAS_VALIDAS_LOWER.get(nombre.split(' ')[0].lower(), nombre.split(' ')[0])
