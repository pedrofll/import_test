...
Notas:
- “Patrocinado” puede aparecer en la tarjeta; se elimina del texto antes de parsear.
- “móvil libre” (y variantes) se elimina del título antes de parsear.
- Imagen: se normaliza a una URL con width=600&height=600 cuando es posible.
"""
...
def obtener_productos(url: str, etiqueta_pagina: str) -> List[ProductoECI]:
    soup = BeautifulSoup(fetch_html(url), "html.parser")
    productos: List[ProductoECI] = []
    for card in detectar_cards(soup):
...
def main() -> int:
    ...
    for i, url in enumerate(URLS_PAGINAS, start=1):
        ...
        try:
            productos = obtener_productos(url, etiqueta)
