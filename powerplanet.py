...
def run_import(
    ...
    if not dry_run:
        ...
        if not (wp_url and wc_key and wc_secret):
            raise SystemExit("Faltan credenciales. Para dry-run: omite --wp-url/--wc-key/--wc-secret o usa --dry-run.")
    ...
    try:
        for url in product_urls:
            ...
            # categorías: marca -> subcategoría (modelo)
...
def main() -> None:
    ...
    ap.add_argument("--wp-url", default="", help="URL base de WordPress, ej: https://ofertasdemoviles.com")
    ap.add_argument("--wc-key", default="", help="WooCommerce consumer key")
    ap.add_argument("--wc-secret", default="", help="WooCommerce consumer secret")
    ...
    # Backward compatible: si no hay credenciales -> dry-run automático
