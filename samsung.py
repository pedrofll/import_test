...
def canonical_samsung_import_url(url: str) -> str:
    """Convierte una URL de compra Samsung a la URL base del producto.

    Ejemplo:
      https://www.samsung.com/es/smartphones/galaxy-s26/buy/?modelCode=SM-S942...
      -> https://www.samsung.com/es/smartphones/galaxy-s26
    """
    try:
...
def sincronizar(remotos):
    ...
    for r in remotos:
        try:
            ...
            raw_buy_url = (r.get('buy_url') or r.get('url_oferta_sin_acortar') or r.get('url_imp') or '').strip()
            url_base = canonical_samsung_import_url(
                r.get('url_importada_sin_afiliado') or r.get('url_imp') or raw_buy_url
            )
            url_expandida = raw_buy_url or url_base
            url_con_afiliado = unir_afiliado(url_base, AFF_RAW) if AFF_RAW else url_base
            url_oferta = acortar_url(url_con_afiliado)

            print(f"10) Enlace Importado: {mask_url(url_base)}", flush=True)
            print(f"11) Enlace Expandido: {mask_url(url_expandida)}", flush=True)
            ...
            data = {
                ...
                'meta_data': [
                    ...
                    {'key': 'url_oferta_sin_acortar', 'value': url_expandida},
